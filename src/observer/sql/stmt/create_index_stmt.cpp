...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  const std::vector<std::string> attr_list = create_index.attr_names;

  for(auto &s: attr_list){
    if (is_blank(s.c_str())){
      LOG_WARN("invalid argument. db=%p, table_name=%p, index name=%s, attr_name=%s", 
      db, table_name, create_index.index_name.c_str(), s.c_str() );
    }
  }
